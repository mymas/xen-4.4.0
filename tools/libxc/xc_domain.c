...
uint64_t do_getCR3(xc_interface *xch)
{
    ...
    hypercall.op     = __HYPERVISOR_do_getCR3;
...
int do_conv_ept(xc_interface *xch, uint64_t paddr, unsigned long l1gfn)
{
    DECLARE_HYPERCALL;
    //DECLARE_HYPERCALL_BOUNCE(arg, len, XC_HYPERCALL_BUFFER_BOUNCE_BOTH);
    uint64_t ret = -EINVAL;

    hypercall.op     = __HYPERVISOR_do_conv_cr3;
    hypercall.arg[0] = paddr;
    hypercall.arg[0] = l1gfn;
    ret = do_xen_hypercall(xch, &hypercall);
...
int xc_get_cr3(xc_interface *xch, uint64_t *cr3)
{
    DECLARE_HYPERCALL;
    ...
    arg = xc_hypercall_buffer_alloc(xch, arg, sizeof(*arg));
    if (arg == NULL) {
        PERROR("Could not allocate memory for get_cr3 hypercall");
        return -1;
    }
...
int xc_conv_ept(xc_interface *xch, unsigned long l2gfn, unsigned long *l1gfn)
{
    ...
    arg = xc_hypercall_buffer_alloc(xch, arg, sizeof(*arg));
    if (arg == NULL) {
        PERROR("Could not allocate memory for conv_ept hypercall");
        return -1;
    }
