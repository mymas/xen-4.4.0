...
#ifdef MIYAMA
uint32_t nept_walk_tables2(struct domain *d, uint64_t eptp,
                                  unsigned long l2ga, ept_walk_t *gw)
{
    int lvl;
    p2m_type_t p2mt;
    uint32_t rc = 0, ret = 0, gflags;
    struct p2m_domain *p2m = d->arch.p2m;
    gfn_t base_gfn = _gfn(eptp >> PAGE_SHIFT);
    mfn_t lxmfn;
    ept_entry_t *lxp = NULL;
...
int nept_translate(struct domain *d, uint64_t eptp, paddr_t l2ga,
                   unsigned long *l1gfn)
{
    uint32_t rc;
    ept_walk_t gw;
    ...
    *l1gfn = INVALID_GFN;
    ...
    rc = nept_walk_tables2(d, eptp, l2ga, &gw);
    switch ( rc )
    {
    case EPT_TRANSLATE_SUCCEED:
        *l1gfn = gw.lxe[0].mfn;
