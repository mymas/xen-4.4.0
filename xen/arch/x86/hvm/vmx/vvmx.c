...
void init_guest_vm(struct VM_LIST *gvm);
...
/* initialize a list and a spinlock when guest_vm is created */
void init_guest_vm(struct VM_LIST *gvm)
{
	INIT_LIST_HEAD(&gvm->pkt_list);
	spin_lock_init(&gvm->pkt_list_lock);
}

/* called from ultracall for receiving a packet */
void save_packet(struct VM_LIST *gvm, unsigned long mfn, unsigned int offset,
		unsigned int size)
{
	struct packet *pkt;
	unsigned char *addr;
	...
	pkt = xmalloc_bytes(sizeof(struct packet) + size);
	pkt->size = size;
	...
	memcpy(pkt->data, addr + offset, size);
	unmap_domain_page(addr);

	spin_lock(&gvm->pkt_list_lock);
	list_add_tail(&pkt->list, &gvm->pkt_list);
	spin_unlock(&gvm->pkt_list_lock);
}

/* new hypercall for obtaining packets */
long do_get_packets(unsigned long tag, XEN_GUEST_HANDLE(uchar) data,
		XEN_GUEST_HANDLE(uint) sizes)
{
	struct VM_LIST *gvm;
//	unsigned int *sizes;
//	unsigned char *data;
	...
	gvm = VM_list_search_by_domtag(tag);

	for (i = 0; i < MAX_PKT; i++) {
		spin_lock(&gvm->pkt_list_lock);

		/* no more packets */
		if (list_empty(&gvm->pkt_list)) {
			spin_unlock(&gvm->pkt_list_lock);
			break;
		}

		pkt = list_entry(gvm->pkt_list.next, struct packet, list);

		/* exceed the max packet size */
		if (total + pkt->size > MAX_SIZE) {
			spin_unlock(&gvm->pkt_list_lock);
			break;
		}
		...
		if (copy_to_guest_offset(data, total, (unsigned char *)pkt->data, pkt->size)) {
			list_add(&pkt->list, &gvm->pkt_list);
			if (i == 0)
				return -EFAULT;
			else
				return i - 1;  /* return only success packets */
		}

		if (copy_to_guest_offset(sizes, i, &pkt->size, sizeof(unsigned int))) {
			list_add(&pkt->list, &gvm->pkt_list);
			if (i == 0)
				return -EFAULT;
			else
...
#ifdef MIYAMA
	shadow_cntrl |= CPU_BASED_CR3_LOAD_EXITING;
#endif
...
/* l2ga: guest physical address */
static unsigned long l2_to_l1(struct vcpu *v, paddr_t l2ga)
{
	unsigned long l1gfn;
	...
	nept_translate_l2ga(v, l2ga, &page_order, 0, &l1gfn, &p2m_acc,
			&exit_qual, &exit_reason);

	return l1gfn;  /* host physical frame number or INVALID_GFN */
}

#if 0
static int ffs8(uint8_t x)
{
	int i;
...
static int ffs16(uint16_t x)
{
	uint8_t h = x >> 8, l = x;
	return l ? ffs8(l) : h ? ffs8(h) + 8 : 0;
}

static int ffs32(uint32_t x)
{
	uint16_t h = x >> 16, l = x;
	return l ? ffs16(l) : h ? ffs16(h) + 16 : 0;
}

static int ffs64(uint64_t x)
{
	uint32_t h = x >> 32, l = x;
	return l ? ffs32(l) : h ? ffs32(h) + 32 : 0;
}

#define PTE_PSE 0x80

/* return host physical frame number */
static unsigned long translate_l2_address(struct vcpu *v, unsigned long l2va)
{
	struct nestedvcpu *nvcpu = &vcpu_nestedhvm(v);
	void *vvmcs = nvcpu->nv_vvmcx;
	u64 guest_cr3;
	unsigned long l2ga, l1gfn, mfn;
	uint64_t mask, pte = 0;
	int level;
	void *map;
	struct page_info *page;

	guest_cr3 = __get_vvmcs(vvmcs, GUEST_CR3);
	l2ga = guest_cr3 & ~0xfffull;

	l2va &= 0x0000ffffffffffffull;
	mask  = 0x0000ff8000000000ull;

	/* Walk the pagetables */
	for (level = 4; level > 0; level--) {
		l2ga += ((l2va & mask) >> (ffs64(mask) - 1)) * 8;

		l1gfn = l2_to_l1(v, l2ga);
		if (l1gfn == INVALID_GFN)
			return 0;

		//map = map_domain_gfn(v->domain, l1gfn);

		page = get_page_from_gfn(v->domain, l1gfn, NULL, P2M_ALLOC);
		if (page == NULL) {
			printk("page is null\n");
			return 0;
		}
		...
		memcpy(&pte, map + (l2ga & (PAGE_SIZE - 1)), 8);
		...
		l2ga = pte & 0x000ffffffffff000ull;
		if (level == 2 && (pte & PTE_PSE)) {
			mask = ((mask ^ ~-mask) >> 1); /* All bits below first set bit */
			l2ga = ((l2ga & ~mask) | (l2va & mask));
			break;
		}
		mask >>= 9;
	}

	return l2_to_l1(v, l2ga);
}
#endif // 0
	...
	switch (exit_reason) {
		...
		case EXIT_REASON_VMCALL:
#ifdef MIYAMA 
			{
				...
				// ultracall for vm's packet
				if (regs->eax == 1001) {
					unsigned long l2gfn = regs->rdi;  // "D"
					unsigned int offset = regs->rsi;  // "S"
					unsigned int size = regs->rdx;  // "d"
					unsigned long l1gfn, mfn;
					struct page_info *page;
					//struct guest_vm *gvm = find_from_eptp(eptp);
					struct VM_LIST *gvm;
					target_eptp = nvmx_vcpu_eptp_base(v);

					l1gfn = l2_to_l1(v, l2gfn << PAGE_SHIFT);
					page = get_page_from_gfn(v->domain, l1gfn, NULL, P2M_ALLOC);
					if (page == NULL) {
						printk("page is null\n");
						break;
					...
					mfn = page_to_mfn(page);
					gvm = VM_list_search_by_ept(target_eptp);
					save_packet(gvm, mfn, offset, size);
					//printk("ultracall for vm's packet\n");
					put_page(page);
					break;
				}
				nvcpu->nv_vmexit_pending = 1;
...
#ifdef SHOTA
				{
					//int dir = (qual & 0x08) ? IOREQ_READ : IOREQ_WRITE;

					if (port >= 0x3f8 && port <= 0x3ff /*&& dir == IOREQ_READ*/) {
						nvcpu->nv_vmexit_pending = 0;
						break;
					}
				}
		...
		case EXIT_REASON_CR_ACCESS:
			{
				...
				if ( cr == 3 )
				{
					...
					/* mov to CR3 */
					gpr = VMX_CONTROL_REG_ACCESS_GPR(exit_qualification);
					reg = decode_register(gpr, guest_cpu_user_regs(), 0);
					if (reg)
						guest_cr3 = *reg;
